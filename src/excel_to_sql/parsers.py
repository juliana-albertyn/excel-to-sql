...
class DateTimeParser:
    ...
    def _timed(self, label: str):
        """
        Context manager for timing parsing steps when debug tracing is enabled.
        """        
        class Timer:
            ...
            def __exit__(inner_self, exc_type, exc, tb):
                duration = (time.perf_counter() - inner_self.start) * 1000
                if self.debug_trace and self.logger:
                    self.logger.debug(f"[TIME] {label}: {duration:.2f} ms")
